...
There was a large relative abundance of viruses in the initial metaphlan3 OTU table.  
This is a repeat of the same analysis without virus.  

Check how many reads belong to virus.  
```{r}
count1 = merged_species_table %>% column_to_rownames(var = "SampleID") %>% select(!Genotype:Sampletype)
count2 = merged_species_table  %>% column_to_rownames(var = "SampleID") %>% select(!Genotype:Sampletype) %>% select(!contains("k__Viruses"))

rowSums(count2)/rowSums(count1) #Percent of bacterial reads remaining
```
Some samples had up to ~99% virus.  

Step one is to filter out.  
```{r}
merged_species_table = merged_species_table %>% select(!contains("k__Viruses"))
```
...
Notes:  
* Large difference in the Shannon diversity of 5xFAD females.   
...
Notes:   
* It still looks like the biggest factor associated with microbial composition is sex.  
